...
| `LRAC_SKIP_ENGLISH` | – | `1` で**英語主体の行**（日本語がほぼ無く英字主体）を読み上げない。英語の思考/説明を聞き流したい時に |
| `LRAC_SKIP_ENGLISH_MINLATIN` | `12` | 英語と判定する英字数の下限（小さいほど短い英語も落とす） |
| `LRAC_QUIET` | – | `1` でログ抑制 |

> Windows では追加依存なしで動きます（再生は `System.Media.SoundPlayer`、クリップボード読取は PowerShell 経由）。

> `LRAC_SKIP_ENGLISH` は**行単位**で「日本語がほぼ無く英字が一定数以上」を英語と判定してスキップします。日本語に英単語が混じる行（例「Claude Code は便利」）は残ります。思考ブロック自体は元々取得していません（応答本文に混じる英文を対象）。
